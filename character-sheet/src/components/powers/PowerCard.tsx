  ...
  return (
    <div className="bg-gray-800 rounded border border-gray-700 overflow-hidden">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
      >
        <span className={`text-xs px-1.5 py-0.5 rounded font-mono ${ACTION_COLOR[power.actionType] ?? 'bg-gray-700 text-gray-400'}`}>
          {power.actionType}
        </span>
        <span className="flex-1 text-white font-semibold text-sm">{power.name}</span>
        <span className={`text-xs px-1.5 py-0.5 rounded ${power.tier === 1 ? 'bg-gray-700 text-gray-400' : 'bg-amber-900 text-amber-300'}`}>
          T{power.tier}
        </span>
      ...
      {open && (
        <div className="px-3 pb-3 border-t border-gray-700 pt-2 space-y-2">
          <div className="flex gap-4 text-xs text-gray-400">
            <span>Cost: <strong className="text-white">{power.cost}</strong></span>
            <span>Target: <strong className="text-white">{power.target}</strong></span>
            <span>Range: <strong className="text-white">{power.range}</strong></span>
          </div>
              ...
              {power.upgrades.map((u) => (
                <label key={u.id} className="flex items-start gap-2 text-sm mb-1 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={u.purchased}
                    onChange={() => dispatch({ type: 'TOGGLE_UPGRADE', powerId: power.id, upgradeId: u.id })}
