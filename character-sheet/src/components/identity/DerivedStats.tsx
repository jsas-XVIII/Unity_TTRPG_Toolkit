...
const STATS: { key: keyof DerivedStats; label: string; tooltip: string }[] = [
  ...
  { key: 'maxRecuperations', label: 'Recups',  tooltip: 'Max Recuperations = 2 + floor(MIGHT/2) + class bonuses' },
...
export default function DerivedStats({ derived }: Props) {
  return (
    <div className="bg-gray-900 rounded-lg p-4">
      <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Derived Stats</h2>
      <div className="grid grid-cols-4 gap-2">
        {STATS.map(({ key, label, tooltip }) => (
          <div key={key} className="flex flex-col items-center bg-gray-800 rounded p-2" title={tooltip}>
