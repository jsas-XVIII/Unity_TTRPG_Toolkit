// fixtures.ts — reusable test data.
// Provides a minimal but fully valid Character object so tests don't have to
// construct one from scratch. Override individual fields inline where needed:
//   { ...baseCharacter, name: 'Override Name' }

import type { Character } from '../types/character'

export const baseCharacter: Character = {
  id: 'test-uuid-1234',
  name: 'Aldric Voss',
  race: 'Human',
  className: 'Dreadnought',
  level: 1,
  xp: 0,
  age: '28',
  notes: '',

  attributes: { might: 3, agility: 1, mind: 1, presence: 1 },

  arBonus: 0,
  drBonus: 0,
  hpBonus: 0,

  currentHp: 17,
  fadingStacks: 0,

  primaryResource: { name: 'Fury', current: 0, max: 6, rechargeDie: '1d4' },
  secondaryResource: null,

  recuperationBonus: 0,
  recuperationDie: '1d6',

  corePaths: [],
  powers: [],
  perks: [],
  weapons: [],
  armor: [],
  equippedArtifacts: [],
  artifactCapacity: 2,

  denerim: 0,
  necessities: 0,
  gear: 0,
}
